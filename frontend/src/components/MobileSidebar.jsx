...
export default function MobileSidebar({ onLogout, uiSettings = {} }) {
  ...
  const moreRoutes = ['/settings', ...(!hide_maps ? ['/maps'] : []), ...(!hide_tokens ? ['/tokens'] : [])]
...
function MoreItem({ to, Icon, label, onClick }) {
  return (
    <NavLink to={to} onClick={onClick} style={({ isActive }) => ({ ...moreItemStyle, color: isActive ? 'var(--gold)' : 'var(--text-dim)', textDecoration: 'none' })}>
      <Icon size={18} />
      <span>{label}</span>
    </NavLink>
  )
...
const moreItemStyle = {
  ...
  padding: '12px 24px', fontSize: 16, background: 'none',
}
