...
export default function MapDetailView() {
  ...
  const { imageStyle } = useImageGestures({ onNext, onPrev, containerRef: imagePane, resetKey: mapId })

  useEffect(() => { api.get(`/maps/${mapId}`).then(setMap) }, [mapId])

  if (!map) return <div style={{ padding: 40, textAlign: 'center' }}><Spinner size={32} /></div>
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <a href={mediaUrl(`/maps/${mapId}/file`)} download style={{
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: isMobilePhone ? 'column' : 'row' }}>
        {/* Image pane */}
        <div ref={imagePane} style={{ flex: 1, overflow: 'auto', background: 'var(--bg-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <img
