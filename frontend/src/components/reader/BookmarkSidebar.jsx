...
export default function BookmarkSidebar({ bookId, currentPage, onGoToPage, onClose, refreshKey }) {
  ...
  useEffect(() => {
    setLoading(true)
    api.get(`/bookmarks?book_id=${bookId}`)
      .then(r => { setBookmarks(r); setLoading(false) })
      .catch(() => { setBookmarks([]); setLoading(false) })
  }, [bookId, refreshKey])

  const handleDelete = (id) => {
    api.delete(`/bookmarks/${id}`)
      .then(() => setBookmarks(bms => bms.filter(b => b.id !== id)))
  }
  ...
  const saveEdit = (bm) => {
    api.patch(`/bookmarks/${bm.id}`, { label: editLabel, notes: editNotes })
      .then(() => {
        setBookmarks(bms => bms.map(b => b.id === bm.id ? { ...b, label: editLabel, notes: editNotes } : b))
        setEditingId(null)
      })
  }
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '6px 4px' }}>
        ...
        {!loading && bookmarks?.map(bm => {
          ...
          const displayLabel = bm.label || (bm.selected_text ? bm.selected_text.slice(0, 40) : `Page ${bm.page_number}`)
          return (
            <div
              key={bm.id}
              style={{
                ...
                {isEditing ? (
                  <div onClick={e => e.stopPropagation()}>
                    <input
                      ...
                      onKeyDown={e => { if (e.key === 'Enter') saveEdit(bm); if (e.key === 'Escape') cancelEdit() }}
                      placeholder={bm.selected_text ? bm.selected_text.slice(0, 40) : `Page ${bm.page_number}`}
                      style={{ fontSize: 13, width: '100%', padding: '2px 4px', marginBottom: 4, boxSizing: 'border-box' }}
                    />
                    <textarea
                      value={editNotes}
                  ...
                  <>
                    <div style={{ fontSize: 13, color: isActive ? 'var(--gold)' : 'var(--text-dim)', fontWeight: isActive ? 500 : 400, lineHeight: 1.4 }}>
                      {displayLabel}
                    </div>
                    {bm.notes && (
                      <div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 3, lineHeight: 1.5, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                        {bm.notes}
                      </div>
                    )}
                    {bm.selected_text && (
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, lineHeight: 1.4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                        "{bm.selected_text}"
                      </div>
                    )}
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>p. {bm.page_number}</div>
                  </>
                )}
              </button>
