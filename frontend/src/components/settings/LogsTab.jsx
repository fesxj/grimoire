...
export default function LogsTab() {
  ...
  useEffect(() => {
    ...
    api
      .get(`/logs?level=${level}&limit=${INITIAL_LIMIT}&offset=0`)
      .then((data) => {
        if (cancelled) return
  ...
  return (
    <div>
      <div
        role="toolbar"
        aria-label={t('logs.toolbarAriaLabel')}
        style={{
          display: 'flex',
          alignItems: 'center',
          gap: 10,
      ...
      >
        <div
          role="group"
          aria-label={t('logs.levelGroupAriaLabel')}
          style={{
            display: 'flex',
        ...
        <button
          ...
          style={{
            ...
            color: live ? '#6fcf6f' : 'var(--text-dim)',
            transition: 'all 0.15s',
          }}
        >
          {live ? (
            <LuPause size={13} aria-hidden="true" />
          ) : (
      ...
      <div style={{ position: 'relative' }}>
        ...
        >
          ...
          {loadingMore && (
            <div
              role="status"
              aria-label={t('logs.loadingOlderAriaLabel')}
              style={{
                padding: '6px 12px',
                fontSize: 11,
                color: 'var(--text-muted)',
          ...
          {hasOlder && !loadingMore && (
            <div
              aria-label={t('logs.olderCountAriaLabel', { count: total - entries.length })}
              style={{
                padding: '4px 12px',
                fontSize: 11,
                color: 'var(--text-muted)',
          ...
          {filtered.length === 0 ? (
            <div
              role="status"
              style={{ padding: 24, fontSize: 13, color: 'var(--text-muted)', textAlign: 'center' }}
            >
              {search ? (
            ...
            <div role="rowgroup">
              {filtered.map((e, i) => (
                <LogRow key={`${e.timestamp}-${i}`} entry={e} searchQuery={search} />
              ))}
            </div>
          )}
        </div>
