...
export default function TokenCard({ token, onClick, bulkMode, selected, onToggle }) {
  ...
  return (
    <div
      ...
      onMouseEnter={e => { if (!bulkMode && !selected) e.currentTarget.style.borderColor = 'var(--border-light)' }}
    ...
    >
      ...
      {bulkMode && (
        <div style={{
          ...
          {selected && <span style={{ fontSize: 11, color: 'var(--bg-deep)', fontWeight: 700 }}>✓</span>}
        </div>
      )}
      {token.is_explicit && (
        <div style={{
